...
Game.prototype.placeRandomWall = function(board) {
    ...
    wall.place();
    if (array[0] == 'w') {
        this.stack.push([wall, [wall.x, wall.y],
            [array[2] + 1 - 0.5, (1 + array[1]) / 2 + 0.6],
            [array[2] + 1, (1 + array[1]) / 2]
        ]);
        this.changePosPeca((array[2] + 1) - 0.5, (1 + array[1]) / 2 + 0.6, wall);
    } else {
        this.stack.push([wall, [wall.x, wall.y],
            [(array[2] + 1) / 2 + 0.4, (array[1] + 1) / 2],
            [(array[2] + 1) / 2, (1 + array[1]) / 2]
        ]);
...
Game.prototype.whoMoved = function(board) {
    ...
    this.stack.push([peca, [peca.x, peca.y],
        [peca.realx, peca.realy]
    ]);
